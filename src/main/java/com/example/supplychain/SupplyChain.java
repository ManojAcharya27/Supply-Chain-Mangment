...
import java.sql.SQLException;
 ...
 public  class SupplyChain extends Application {
            ...
            public void handle(ActionEvent actionEvent) {
                ...
                if(loggedIn==false)
                {
                    ...
                    if(product!=null)
                    {
                        ...
                        System.out.println("Please selected product");
                    }
                }
            }
        });
    ...
    private GridPane hearBar(){
       ...
       // gridPane.getChildren().remove(3,0);
        ...
        cartButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent actionEvent) {
                 bodyPane.getChildren().clear();
                bodyPane.getChildren().add(cartDetails.getAllCartProducts());
                Product product=productDetails.getSelectedProduct();
                //cartDetails.getAllCartProducts();
                if(product!=null&&Order.insertingIntoCart()) {
                       bodyPane.getChildren().add(cartDetails.getAllCartProducts());
    ...
    private GridPane loginPage(){
        ...
        emailTextField.setPromptText("Please Enter your email id ");
        ...
        loginButton.setOnAction(new EventHandler<ActionEvent>() {
            @Override
            public void handle(ActionEvent actionEvent) {
                String emailMessage=emailTextField.getText();
                String  password=passwordField.getText();
                if(login.customerLogin(emailMessage,password))
                {
                    loginLabel.setText("Welcome : "+emailMessage);
                    bodyPane.getChildren().clear();
                    bodyPane.getChildren().add(productDetails.getAllProducts());
                    loggedIn=true;
                 //   messageLabel.setText("Login Successful");
                }else{
                    messageLabel.setText("Invalid Password and email id");
                }
               // messageLabel.setText("email - "+emailMessage+" && pass - "+password);
            }
        });
        GridPane gridPane=new GridPane();
    ...
    public void start(Stage stage) throws IOException {
       // FXMLoader fxmlLoader = new FXMLLoader(HelloApplication.class.getResource("hello-view.fxml"));
